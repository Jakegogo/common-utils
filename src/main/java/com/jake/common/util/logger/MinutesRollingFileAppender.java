...
public class MinutesRollingFileAppender extends FileAppender {
...
/**
  ...
  public MinutesRollingFileAppender() {
  }
  ...
  /**
    ...
    */
  public MinutesRollingFileAppender (Layout layout, String filename,
  ...
  public void activateOptions() {
    ...
    this.orignFileName = fileName;
    rc = new RollingCalendar(minutesInterval);
  ...
  int computeCheckPeriod() {
    RollingCalendar rollingCalendar = new RollingCalendar(gmtTimeZone, Locale.getDefault(), minutesInterval);
  ...
  */
  void rollOver() throws IOException {
    ...
    int curMinutes = rollTime.get(Calendar.MINUTE);
	int floorMinutes = curMinutes / this.minutesInterval * this.minutesInterval;
    rollTime.set(Calendar.MINUTE, floorMinutes);
    
//    log.debug(" ＝＝＝＝＝ log4j rolled file time :" + rollTime.getTime());
    
    String datedFilename = orignFileName + sdf.format(rollTime.getTime());
...
class RollingCalendar extends GregorianCalendar {
  ...
  int type = MinutesRollingFileAppender.TOP_OF_TROUBLE;

  private int minutesInterval;
 
  RollingCalendar(int minutesInterval) {
    super();
    this.minutesInterval = minutesInterval;
  }  

  RollingCalendar(TimeZone tz, Locale locale, int minutesInterval) {
    super(tz, locale);
    this.minutesInterval = minutesInterval;
  }  
  ...
  public Date getNextCheckDate(Date now) {
    ...
    switch(type) {
    case MinutesRollingFileAppender.TOP_OF_MINUTE:
	this.set(Calendar.SECOND, 0);
	this.set(Calendar.MILLISECOND, 0);
	int curMinutes = this.get(Calendar.MINUTE);
	int floorMinutes = (curMinutes + this.minutesInterval) / this.minutesInterval * this.minutesInterval;
	this.add(Calendar.MINUTE, floorMinutes - curMinutes);
	break;
    case MinutesRollingFileAppender.TOP_OF_HOUR:
	this.set(Calendar.MINUTE, 0);
	this.set(Calendar.SECOND, 0);
	this.set(Calendar.MILLISECOND, 0);
	this.add(Calendar.HOUR_OF_DAY, 1);
	break;
    case MinutesRollingFileAppender.HALF_DAY:
	this.set(Calendar.MINUTE, 0);
	this.set(Calendar.SECOND, 0);
	this.set(Calendar.MILLISECOND, 0);
	...
	break;
    case MinutesRollingFileAppender.TOP_OF_DAY:
	this.set(Calendar.HOUR_OF_DAY, 0);
	this.set(Calendar.MINUTE, 0);
	this.set(Calendar.SECOND, 0);
	this.set(Calendar.MILLISECOND, 0);
	...
	break;
    case MinutesRollingFileAppender.TOP_OF_WEEK:
	...
	break;
    case MinutesRollingFileAppender.TOP_OF_MONTH:
